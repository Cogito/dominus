...
Meteor.publish('networth_rankings', function() {
	if(this.userId) {
		return Meteor.users.find({}, {sort: {networth: -1}, fields: {
			income:1,
			username:1,
			castle_id:1,
			x:1,
...
Meteor.publish('income_rankings', function() {
	if(this.userId) {
		return Meteor.users.find({}, {sort: {income: -1}, fields: {
			income:1,
			username:1,
			castle_id:1,
			x:1,
...
Meteor.publish('ally_rankings', function() {
	if(this.userId) {
		return Meteor.users.find({}, {sort: {num_allies_below: -1}, fields: {
			income:1,
			username:1,
			castle_id:1,
			x:1,
...
Meteor.publish('vassal_rankings', function() {
	if(this.userId) {
		return Meteor.users.find({}, {sort: {num_vassals: -1}, fields: {
			income:1,
			username:1,
			castle_id:1,
			x:1,
...
Meteor.publish('losses_rankings', function() {
	if(this.userId) {
		return Meteor.users.find({}, {sort: {losses_worth: -1}, fields: {
			income:1,
			username:1,
			castle_id:1,
			x:1,
...
Meteor.publish('dominus', function() {
	if(this.userId) {
		return Meteor.users.find({is_dominus: true})
	} else {
		this.ready()
	}
})
